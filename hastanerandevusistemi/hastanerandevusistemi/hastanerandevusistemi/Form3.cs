...
namespace hastaneRandevuSistemi
{
    public partial class Form3 : Form
    {
        ...
        public Form3(Form1 form1, string tc)
        {
            InitializeComponent();
        ...
        private async void Form3_Load(object sender, EventArgs e)
        {
            ...
            labelAdsoyad.Text = doktorAdi;
            lbltc.Text = tc;
            lblbolum.Text = await GetDoktorBolum(tc);
            lblHastane.Text = await GetDoktorHastane(tc);
        ...
        private async Task<string> GetDoktorBolum(string tc)
        {
            string query = "SELECT b.bolum_isim FROM bolumler AS b JOIN doktorlar AS d ON b.bolum_id = d.bolum_id WHERE d.tc_no = @tc";
        ...
        private async Task<string> GetDoktorHastane(string tc)
        {
            string query = "SELECT h.hastane_isim FROM hastaneler AS h JOIN doktorlar AS d ON h.hastane_id = d.hastane_id WHERE d.tc_no = @tc";
        ...
        private void RandevulariGetir(int page)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                WHERE DATE(r.randevu_tarih) = CURDATE() AND r.doktor_id = @doktor
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        string hastatc = reader["tc_no"].ToString();
                        ...
                        string bilgi = $" {hastatc}    {adsoyad}    {dogumtarihi:dd.MM.yyyy}     {cinsiyet}    {tarih:dd.MM.yyyy}/{saat:HH:mm}";
                        var panel = OlusturPanel(bilgi, tarih, saat.ToString("HH:mm"));
                        Label buton = new Label
                        {
                            Text = "İşlem",
                            Width = 40,
                            Height = 20,
                        ...
                        buton.Click += (s, e) =>
                        {
                            LoadUserControlIcerik(new hastaliklarimilaclarim(hastatc));
                        };
                        buton2.Click += (s, e) =>
                        {
                        ...
                        buton.Location = new Point(panel.Width - buton.Width - 50, 10);
                        panel.Controls.Add(buton);
                        buton2.Location = new Point(panel.Width - buton.Width -5, 10);
                        panel.Controls.Add(buton2);
        ...
        {
            LoadUserControlIcerik(new dokgecmisrandevular(tc));
        }

        private void btnGelecekrand_Click(object sender, EventArgs e)
        {
            LoadUserControlIcerik(new dokGelecekrandevular(tc));
        }

        private void btnAnasayfa_Click(object sender, EventArgs e)
        {
            LoadUserControlIcerik(new bugununRandevulari(tc));
        }
    }
}
